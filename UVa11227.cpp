...
int main()
{
	...
	while (t--)
	{
		...
		else
		{
			...
			//		form a line connecting the points and count how many gnus are on the line
			//		can afford O(n^3), so just check every OTHER other point if it is on the line
			for (auto i = gnus.begin(); i != gnus.end(); ++i)
			{
				for (auto j = gnus.begin(); j != gnus.end(); ++j)
				{
					if (j == i)
						continue;
					point p = *i, q = *j; // current line goes through p and q
					int curaligned = 2;
					for (auto k = gnus.begin(); k != gnus.end(); ++k)
					{
						if (k == j || k == i)
							continue;
						if (collinear(*k, p, q))
