...
#define MAX_N 610                         // second approach: O(n log n)
...
int main()
{
	...
	while (scanf("%s\n", T) != EOF)
	{
		...
		for (int i = 0; i < n; ++i)
		{
			if (owner(SA[i]) != owner(SA[i-1]) && LCP[i] == ans.first)
			{
				char lcsans[MAX_N];
				memset(lcsans, 0, MAX_N);
				strncpy(lcsans, T + SA[i], ans.first);
				if (answers.find(string(lcsans)) == answers.end() && strcmp(lcsans, "") != 0)
				{
					printf("%s\n", lcsans);
					answers.insert(string(lcsans));
				}
			}
		}
		if (answers.empty())
		...
		memset(P, 0, MAX_N);
		memset(RA, -1, MAX_N);
		memset(tempRA, -1, MAX_N);
		memset(RA, -1, MAX_N);
		memset(tempSA, -1, MAX_N);
		memset(c, -1, MAX_N);
		memset(Phi, -1, MAX_N);
		memset(PLCP, -1, MAX_N);
		memset(LCP, -1, MAX_N);
